...
const LoginForm: FC<PropsType> = ({ setKey }) => {
	...
	return (
		<Wrapper noValidate onSubmit={handleSubmit}>
			...
			<p className="ForgetText" onClick={() => setKey('forgot-pass')}>
				Forget Password?
			</p>
			<Button type="submit" className="mt-2 w-100 text-light border-dark bg-dark FormSubmitBtn">
				Login
			</Button>

			<p className="FormText">
				Dont have any account?
				<span className="text-primary" onClick={() => setKey('registration')}>
					{' '}
					Register
				</span>
			</p>
